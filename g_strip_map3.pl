...
my ( $a1, $a2, $a3, $a4 );
my ( $start_time_interval, $end_time_interval, $start_loop_time );

my ( $PXKM, $MMKM, $IMAGE_LENGTH_MM, $IMAGE_WIDTH_MM, $LENGTHWAYS_MARGIN_MM, $SIDEWAYS_MARGIN_MM, $LENGTHWAYS_MARGIN_KM, $SIDEWAYS_MARGIN_KM );
my ( $LENGTHWAYS_MARGIN_PX, $SIDEWAYS_MARGIN_PX );  
my ( $IMAGE_LENGTH_PX, $IMAGE_WIDTH_PX, $IMAGE_LENGTH_KM, $IMAGE_WIDTH_KM, $MAX_DISTANCE_KM, $MAX_DEVIATION_KM, $MAX_DISTANCE_PX, $MAX_DEVIATION_PX );
...
# the programme is typically used to produce two strips which can be printed side-by-side on a single A4 sheet.
# an A4 sheet is 297 x 210 mm 
# however most printers can't print within 5 mm of each edge reducing the effective size to 287 x 200 mm
# if we allow a 2 mm margin between the two strips then each image will be 287 x 99 mm
#

$PXKM = 200;   # at 1:50,000 scale 1 km is represented by 2 cm on a map; so 100 pixels/cm, 10 pixels/mm or 254 pixels/inch 
$MMKM =  20;

my $scale = 0.94;

###########################################################################
#   define next four variable to set size of plot
###########################################################################

$IMAGE_LENGTH_MM        = 287;
$IMAGE_WIDTH_MM         =  99;

$LENGTHWAYS_MARGIN_MM   =  20;  # 1.0 km on each side
$SIDEWAYS_MARGIN_MM     =  10;  # 0.5 km on each side

###########################################################################
#   calculate dependent dimensions
###########################################################################

$IMAGE_LENGTH_PX    = int($IMAGE_LENGTH_MM * $PXKM / $MMKM);
$IMAGE_WIDTH_PX     = int($IMAGE_WIDTH_MM * $PXKM / $MMKM);

$IMAGE_LENGTH_KM    = $IMAGE_LENGTH_MM / $MMKM;
$IMAGE_WIDTH_KM     = $IMAGE_WIDTH_MM / $MMKM;

$LENGTHWAYS_MARGIN_KM   =  $LENGTHWAYS_MARGIN_MM / $MMKM;  
$SIDEWAYS_MARGIN_KM     =  $SIDEWAYS_MARGIN_MM / $MMKM;  

$LENGTHWAYS_MARGIN_PX   =  int($LENGTHWAYS_MARGIN_KM * $PXKM);  
$SIDEWAYS_MARGIN_PX     =  int($SIDEWAYS_MARGIN_KM * $PXKM);  

$MAX_DISTANCE_KM    = $IMAGE_LENGTH_KM - ( 2.0 * $LENGTHWAYS_MARGIN_KM );  
$MAX_DEVIATION_KM    = $IMAGE_WIDTH_KM - ( 2.0 * $SIDEWAYS_MARGIN_KM );  

$MAX_DISTANCE_PX    = int($MAX_DISTANCE_KM * $PXKM);
$MAX_DEVIATION_PX   = int($MAX_DEVIATION_KM * $PXKM);


print "\$PXKM: $PXKM\t \$MMKM: $MMKM\n\$IMAGE_LENGTH_MM: $IMAGE_LENGTH_MM\t \$IMAGE_WIDTH_MM: $IMAGE_WIDTH_MM\n\$LENGTHWAYS_MARGIN_MM: $LENGTHWAYS_MARGIN_MM \$SIDEWAYS_MARGIN_MM $SIDEWAYS_MARGIN_MM\n";
print "\$IMAGE_LENGTH_PX $IMAGE_LENGTH_PX\t \$IMAGE_WIDTH_PX: $IMAGE_WIDTH_PX\n\$IMAGE_LENGTH_KM: $IMAGE_LENGTH_KM\t \$IMAGE_WIDTH_KM: $IMAGE_WIDTH_KM\n";
print "\$MAX_DISTANCE_KM: $MAX_DISTANCE_KM\t \$MAX_DEVIATION_KM: $MAX_DEVIATION_KM\n\$MAX_DISTANCE_PX: $MAX_DISTANCE_PX\t \$MAX_DEVIATION_PX: $MAX_DEVIATION_PX \n";
...
$max_distance  = $MAX_DISTANCE_KM;  # km
$max_deviation =  $MAX_DEVIATION_KM;   # km



print "max_distance: $max_distance  max_deviation: $max_deviation\n";
...
while ( $start_index < $n-4 ) {  # to ensure finishes!!!
    ...
    printf("TIMER: read and decode GPX file: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
    $start_loop_time = $end_time_interval;
    
    if ( $debug ) { print "START LOOP: start_index: $start_index  end_index: $end_index      "; }
    ...
    $end_time_interval = time();
    printf("TIMER: determined max rectangle: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
    ...
    $border_km = 4.0;  # add border all around so no gaps when rotated
    ...
    $end_time_interval = time();
    printf("TIMER:      generated composite: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
    ...
    $end_time_interval = time();
    printf("TIMER:        draw track on map: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
...
#}            
    ...
    $end_time_interval = time();
    printf("TIMER:            rotated image: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
   ...
   #---- now trim rotated image to final rectangular size ----
    ...
    $x = int($x - $LENGTHWAYS_MARGIN_PX);
    $y = int($y - $SIDEWAYS_MARGIN_PX);  
    
    $width = $IMAGE_LENGTH_PX;
 ...
 #   print "guess: \$im, 0, 0,   680, 1200, 2430, 830 );    ## copy(sourceImage, dstX, dstY, srcX, srcY, width, height) object\n";    
   ...
   $end_time_interval = time();
    printf("TIMER:               trim image: %6.2f sec\n", $end_time_interval - $start_time_interval);
    $start_time_interval = $end_time_interval;
    ...
    $end_time_interval = time();
    printf("TIMER:      annotate and rotate: %6.2f sec\n", $end_time_interval - $start_time_interval);
    printf("TIMER:               total time: %6.2f sec\n", $end_time_interval - $start_loop_time);
    $start_time_interval = $end_time_interval;
...
##################################################################################
    ...
    for ( $index = $start_index ; $index < $end_index ; $index++ ) {
        ...
        if ( $distance > $max_distance  ) {
                if ( $debug ) { print sprintf("INT: MAX DIST EXCEDED d: %.3f max_d %.3f\n", $distance, $max_distance ); }
        ...
        for ( $i = $start_index ; $i < $index ; $i++ ) {
            ...
            if ( $deviation_along > $max_deviation ) {
                if ( $debug ) { print sprintf("INT: MAX DEVIATION EXCEEDED  d: %.3f max_d %.3f dev: %.3f max_dev: %.3f\n",  $distance, $max_distance, $deviation, $max_deviation ); }
