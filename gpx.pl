...
#=========================================================================
#
#   reads a GPX file and removes 'stationary' points
#   the GPX file contains one or more segments each containing a track
#
#	it implements two algorithms to clean up the track -
#		A(B)C, and
#		Clusterng
#
#	A(B)C
#   	it examines each set of 3 consecutive points in each track segment - A, B and C 
#		point B is removed if any of the following apply
#   	  - if point B is within a certain minimum distance (say 15 metres) of both A and C
#		  - if the speeds between both A->B and B->C are both below a certain speed threshold (say 0.45 m/s) 
#		  - if one of the speeds A->B or B->C is above some speed threshold
#		this means that both the times at which a track stops and starts are both retained
#
#	Clustering
#		each point is examined to see how many other points are 'close' to it
...
my ( $speed_ab, $speed_bc, $last_date_time );
my ( $start, $n_remove );
...
$abc_max_speed_threshold	= 20.0;   # 20.0;		# in metres/sec	~ 45 mph
...
$usage = 'USAGE: gpx.pl <input_file> <output_gpx_file> <output_csv_file>';
...
print "file length = " . length($data) . " bytes\n";
...
open (CSV, ">$output_csv_file") || die("unable to open $output_csv_file");

print CSV "Version 2:CSV\n";
print CSV "Datum:,WGS-84\n";
print CSV "ZoneOffset:,0.0\n";
print CSV '"Type","Name|Color|Visibility","Lat","Long","Month#","Day#","Year","Hour","Min","Sec","Altitude (Feet)","Depth (Feet)","Temp Deg C","Track Point Name","Units","DP","DS","DT","VP (Mi/hr)","TP"' . "\n";
...
while ( $data =~ m!<trkseg>(.*?)</trkseg>!sg ) {        # find each segment
    ...
    while ( $segment =~ m!<trkpt\s(.*?)</trkpt>!sg ) {       # find all data points within this segment
		...
		if ( $n_pts_in_seg >= 1 ) {  		# skip first two points in each new segment
            if ( $date_time ne $last_date_time ) {   # skip if repeat time ...
...
#		print "a: $a  b: $b  c: $c\n";
        ...
        if ( $seconds_ab == 0 ) { $seconds_ab = 0.001; }    # avoid divide by zero
        if ( $seconds_bc == 0 ) { $seconds_bc = 0.001; }
                
        $speed_ab = $distance_ab / $seconds_ab;      # m/s
        $speed_bc = $distance_bc / $seconds_bc;      # m/s
...
##		if ( $k >= 2 ) {  # skip first two points in each segment
...
##		}
		...
		print CSV "T,$date_time,$latitude,$longitude,$month,$day,$year,$hour,$minute,$second,$elevation,1,1,\"1\",1,1,1,1,1,1\n";
